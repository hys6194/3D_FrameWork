...
_bool Animation::Update_TransformationMatrix(const vector<class Bone*>& pBone, _float fTimeDelta, _bool bIsLoop, _float* pCurrentTrackPosition, vector<_uint>& vecKeyFrameIndex, _float* pPreTrackPosition, vector<_uint>* vecPreAnimKeyFrameIndex)
{
    if(nullptr == pPreTrackPosition)
    {
        // 현재 재생위치를 구한다
