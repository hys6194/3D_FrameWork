...
CVIBuffer_Flat_Terrain::CVIBuffer_Flat_Terrain(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
...
CVIBuffer_Flat_Terrain::CVIBuffer_Flat_Terrain(const CVIBuffer_Flat_Terrain& Prototype)
	: CVIBuffer{ Prototype }
{
}

HRESULT CVIBuffer_Flat_Terrain::Initialize_Prototype(const _tchar* pHeightMapFilePath)
...
#pragma region VERTEXBUFFER
	...
	for (size_t i = 0; i < m_iNumVerticesZ; i++)
	{
		for (size_t j = 0; j < m_iNumVerticesX; j++)
		{
			...
			pVertices[iIndex].vPosition = _float3(j, (pPixel[iIndex] & 0x000000ff) * 0.f, i);
...
HRESULT CVIBuffer_Flat_Terrain::Initialize(void* pArg)
{
	return S_OK;
}

CVIBuffer_Flat_Terrain* CVIBuffer_Flat_Terrain::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _tchar* pHeightMapFilePath)
{
	CVIBuffer_Flat_Terrain* pInstance = new CVIBuffer_Flat_Terrain(pDevice, pContext);
...
CComponent* CVIBuffer_Flat_Terrain::Clone(void* pArg)
{
	CComponent* pInstance = new CVIBuffer_Flat_Terrain(*this);
...
void CVIBuffer_Flat_Terrain::Free()
{
	__super::Free();
